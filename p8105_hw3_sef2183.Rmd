...
(i) note that a scatterplot may not be the best option)

```{r problem 3 tmax vs tmin and snowfall plots}
i_p = ny_noaa_df %>%
  filter(month == "01") %>% 
  ggplot(aes(x = tmin, y = tmax)) + geom_point() + # NOT SCATTER - CHANGE
  ggtitle("Plot of max temperature vs minimum temperature")

ii_p = ny_noaa_df %>%
  filter(snow < 0 | snow > 100) %>% 
  group_by(year) %>% 
  ggplot(aes(x = year, y = snow)) + 
  theme(axis.text.x = element_text(angle = 90,hjust = 1)) + geom_histogram() +
  ggtitle("Distribution of snowfall values greater than 0 and less than 100 by year")
